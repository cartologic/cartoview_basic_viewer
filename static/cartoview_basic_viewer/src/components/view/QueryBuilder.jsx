...
class QueryBuilder extends React.Component {
    render() {
        const { classes, removeComponent, createQueryPanel, getFeatureTableData, components, createQueryRef } = this.props
        return (<div className={classes.root}>
            <div className={classes.queryBuilderTools}>
                <IconButton onClick={createQueryPanel} className={classes.button} aria-label="Add">
                    <AddIcon />
                </IconButton>
                {components.length > 0 && <IconButton onClick={() => getFeatureTableData(undefined, undefined, undefined, false)} className={classes.button} aria-label="Search">
                    <Searchcon />
                </IconButton>}
                {components.length > 0 && <IconButton onClick={() => getFeatureTableData(undefined, undefined, undefined, true)} className={classes.button} aria-label="Download with Filters">
                    <Downloadcon />
                </IconButton>}
            </div>
            <div className={classes.queryItemsContainer}>
                {components.map((cmp, index) => <div className={classes.item} key={index}>
                    <cmp.Component  {...cmp.props} innerRef={(node) => createQueryRef(cmp.ref, node)} />
